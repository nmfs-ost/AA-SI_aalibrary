...
def main():
    ...
    if mode == "Search By NCEI Vessels":
        ...
        if operation == "Download .raw":
            ...
            logger.debug(f"Running command: aa-raw --file_name {folder_name} --ship_name {ship_name} --survey_name {survey} --echosounder {echosounder} --file_download_directory {folder_name} from directory: {os.getcwd()} in python environment: {os.environ.get('VIRTUAL_ENV', 'No virtual environment')}")
            subprocess.run([
                "aa-raw",
                "--file_name", file_name,
                "--file_type", "raw",
