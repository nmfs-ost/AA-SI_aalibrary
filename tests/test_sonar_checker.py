...
def test_is_EK80_ek80_files():
    ...
    for test_file_path in ek80_files:
        assert is_EK80(test_file_path, storage_options={}) is True
...
def test_is_EK80_non_ek80_files():
    ...
    for test_file_path in ek60_files:
        assert is_EK80(test_file_path, storage_options={}) is False
...
def test_is_EK60_ek60_files():
    ...
    for test_file_path in ek60_files:
        assert is_EK60(test_file_path, storage_options={}) is True
...
def test_is_EK60_non_ek60_files():
    ...
    for test_file_path in ek80_files:
        assert is_EK60(test_file_path, storage_options={}) is False
...
def test_is_ER60_er60_files():
    ...
    # Check that each file in ek60 is identified as EK60
    for test_file_path in ek60_files:
        assert is_ER60(test_file_path, storage_options={}) is True
...
def test_is_ER60_non_er60_files():
    ...
    # Check that each file in ek80 is not identified as EK60
    for test_file_path in ek80_files:
        assert is_ER60(test_file_path, storage_options={}) is False
...
def test_is_AZFP6_valid_files():
    ...
    for test_file_path in azfp_files:
        assert is_AZFP6(test_file_path) is True
...
def test_is_AZFP6_invalid_files():
    ...
    for test_file_path in non_azfp_files:
        assert is_AZFP6(test_file_path) is False


@pytest.fixture(scope="session")
def test_is_AZFP_valid_files():
    """Test that XML files with <InstrumentType string="AZFP"> are identified
    as valid AZFP files."""
    # Collect all valid XML files in the test directory
    valid_files = glob.glob("echopype/test_data/azfp/*.xml") + glob.glob(
        "echopype/test_data/azfp/*.XML"
    )

    for test_file_path in valid_files:
        assert is_AZFP(test_file_path) is True


@pytest.fixture(scope="session")
def test_is_AZFP_invalid_files():
    """Test that XML files without <InstrumentType string="AZFP"> are not
    identified as valid AZFP files."""
    ...
    for test_file_path in invalid_files:
        assert is_AZFP(test_file_path) is False
...
def test_is_AD2CP_valid_files():
    ...
    for test_file_path in ad2cp_files:
        assert is_AD2CP(test_file_path) is True
...
def test_is_AD2CP_invalid_files():
    ...
    for test_file_path in non_ad2cp_files:
        assert is_AD2CP(test_file_path) is False
